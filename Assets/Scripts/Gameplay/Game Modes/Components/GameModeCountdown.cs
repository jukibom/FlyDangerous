...
namespace Gameplay.Game_Modes.Components {
    public class GameModeCountdown {
         ...
         */
        public IEnumerator CountdownWithSound(float countdownTimeSeconds, Action<float> onSecondsRemaining,float speed = 1f) {
            ...
            if (edgeTime != 0) {
                yield return YieldExtensions.WaitForFixedFrames(YieldExtensions.SecondsToFixedFrames(edgeTime/speed));
            ...
            while (remainingSeconds > 0) {
                yield return YieldExtensions.WaitForFixedFrames(YieldExtensions.SecondsToFixedFrames(1f/speed));
                remainingSeconds -= 1;
