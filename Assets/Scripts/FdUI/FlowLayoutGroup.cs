...
/// Fixed right / bottom padding and ability to maintain consistent sizes (ForceAllSameDimension) by Jay Faulkner
...
namespace FdUI {
    ...
    public class FlowLayoutGroup : LayoutGroup {
        ...
        public bool ForceAllSameDimension;
        ...
        public float SetLayout(int axis, bool layoutInput) {
            ...
            // Force same dimension override inspects the bar size of the first element and applies that to the last bar IF it isn't the only bar
            var finalBarSize = workingSize;
            if (ForceAllSameDimension && rectChildren.Count > 0 && rectChildren.Count != _itemList.Count)
                finalBarSize = (startAxis == Axis.Horizontal ? rectChildren[0].rect.width : rectChildren[0].rect.height) * _itemList.Count +
                               spacingBetweenElements * Mathf.Max(_itemList.Count - 1, 1);

            if (!layoutInput) {
                if (startAxis == Axis.Horizontal) {
                    var newOffset = CalculateRowVerticalOffset(groupHeight, offset, currentBarSpace);
                    currentBarSize -= spacingBetweenElements;
                    LayoutRow(_itemList, currentBarSize, currentBarSpace, finalBarSize - (ChildForceExpandWidth ? 0 : spacingBetweenElements) - padding.right,
                        padding.left, newOffset, axis);
                }
                else if (startAxis == Axis.Vertical) {
                    var newOffset = CalculateColHorizontalOffset(groupWidth, offset, currentBarSpace);
                    currentBarSize -= spacingBetweenElements;
                    LayoutCol(_itemList, currentBarSpace, currentBarSize, finalBarSize - (ChildForceExpandHeight ? 0 : spacingBetweenElements) - padding.bottom,
        ...
        protected void LayoutRow(IList<RectTransform> contents, float rowWidth, float rowHeight, float maxWidth, float xOffset, float yOffset, int axis) {
            ...
            for (var j = 0; j < _itemList.Count; j++) {
                ...
                if (j < _itemList.Count - 1) xPos += rowChildWidth + SpacingX;
            }
        }
        ...
        protected void LayoutCol(IList<RectTransform> contents, float colWidth, float colHeight, float maxHeight, float xOffset, float yOffset, int axis) {
            ...
            for (var j = 0; j < _itemList.Count; j++) {
                ...
                if (j < _itemList.Count - 1) yPos += rowChildHeight + SpacingY;
            }
        }
