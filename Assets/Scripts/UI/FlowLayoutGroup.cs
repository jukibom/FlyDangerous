...
namespace UnityEngine.UI.Extensions
{
	...
	public class FlowLayoutGroup : LayoutGroup
	{
		...
		public bool ForceAllSameDimension = false;
		...
		public float SetLayout(int axis, bool layoutInput)
		{
			...
			if (ForceAllSameDimension && rectChildren.Count > 0 && rectChildren.Count != _itemList.Count) {
				finalBarSize = (startAxis == Axis.Horizontal ? rectChildren[0].rect.width : rectChildren[0].rect.height) * _itemList.Count + spacingBetweenElements * Mathf.Max(_itemList.Count - 1, 1);
			}
			...
			if (!layoutInput) {
				if (startAxis == Axis.Horizontal) {
					...
					currentBarSize -= spacingBetweenElements;
					LayoutRow (_itemList, currentBarSize, currentBarSpace, finalBarSize - (ChildForceExpandWidth ? 0 : spacingBetweenElements) - padding.right, padding.left, newOffset, axis);
				}else if (startAxis == Axis.Vertical) {
					float newOffset = CalculateColHorizontalOffset(groupWidth, offset, currentBarSpace);
					currentBarSize -= spacingBetweenElements;
					LayoutCol(_itemList, currentBarSpace, currentBarSize, finalBarSize - (ChildForceExpandHeight ? 0 : spacingBetweenElements) - padding.bottom, newOffset, padding.top, axis);
				}
			}
